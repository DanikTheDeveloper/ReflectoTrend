...
import { StraightLine } from "@react-financial-charts/series";
...
import { OHLCTooltip, MovingAverageTooltip, MACDTooltip } from "@react-financial-charts/tooltip";
import { ema, sma, macd } from "@react-financial-charts/indicators";
import { Brush } from "@react-financial-charts/interactive";
import { last, isDefined } from "@react-financial-charts/core";
...
class CandlestickChart extends React.Component {
    ...
    handleChangeViewRange = (startIdx, endIdx) => {
        this.setState({
            ...this.state,
    ...
    render() {
        const { type, width, ratio, stockName, clamp, plotResults, activePage } = this.props;
        ...
        const height = this.props.macd ? 1050 : 650;
            ...
            <>
                ...
                >
                    ...
                    >
                        ...
                        <YAxis axisAt="left" orient="left" ticks={10} zoomEnabled={false} showTicks={true}
                            className="react-financial-charts-y-axis y-axis"
                        />
                        ...
                        {/* <InteractiveYCoordinate yCoordinateList={this.yCoordinateList} enabled /> */}

                        <Brush
                            ref={this.saveInteractiveNode(1)}
                            enabled={this.props.brush}
                            type={BRUSH_TYPE}
                            onBrush={this.handleBrush1}
                        />
                        <MouseCoordinateX at="bottom" orient="bottom" displayFormat={timeFormat("%Y-%m-%d")} />
                        {plotResults !== undefined &&
                            plotResults.map((result, index) => (
                                <div id={index}>
                                    <StraightLine type="vertical" xValue={result.startIdx}
                                        stroke={activePage - 1 === index ? "red" : "blue"}
                                        strokeWidth={activePage - 1 === index ? 2 : 1}
                                        opacity={activePage - 1 === index ? 1 : 0.5}
                                    />
                                    <StraightLine type="vertical" xValue={result.endIdx}
                                        stroke={activePage - 1 === index ? "red" : "blue"}
                                        strokeWidth={activePage - 1 === index ? 2 : 1}
                                        opacity={activePage - 1 === index ? 1 : 0.5}
                                    />
...
const Charter = withSize({ style: { minHeight: 650, minWidth: 1500 } })(withDeviceRatio()(CandlestickChart));
